...
public class MockStrategyGroovySubscriber implements GroovySubscriber {

    protected final static Logger log = LoggerFactory.getLogger(MockStrategyGroovySubscriber.class);
    ...
    public void subscribe(GroovyConfig groovyConfig) {
        ...
        //版本没有更新，我们认为没有更新
        if (!groovyConfig.getValid() && GroovyCache.contains(groovyConfig)){
            GroovyCache.removeMockStrategy(groovyConfig.getId());
        ...
        if (groovyConfig.getValid() && !GroovyCache.contains(groovyConfig)) {
            try {
                ...
                if (groovyObj instanceof MockInterceptor) {
                    GroovyCache.addMockStrategy(groovyConfig.getId(), (MockInterceptor)groovyObj, groovyConfig.getVersion());
                    log.info("success update groovy script id={} version={}", groovyConfig.getId(),  groovyConfig.getVersion());
                    log.debug("script id={} content={}", groovyConfig.getId(), groovyConfig.getContent());
                } else {
                    log.error("appName={} id={} groovy script is not instance of MockInterceptor", groovyConfig.getAppName(), groovyConfig.getId());
                }
    ...
    public GroovyType getType() {
        return GroovyType.MockInterceptor;
    }
